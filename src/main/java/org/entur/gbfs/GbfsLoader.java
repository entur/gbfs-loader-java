...
public class GbfsLoader {
    ...
    private void createUpdaters() {
        ...
        GBFSFeeds feeds = languageCode == null ? disoveryFileData.getFeedsData().values().iterator().next()
        ...
        for (GBFSFeed feed : feeds.getFeeds()) {
            ...
            if (feedUpdaters.containsKey(feedName)) {
                throw new RuntimeException("Feed contains duplicate url for feed " + feedName + ". " + "Urls: " + feed
                        .getUrl() + ", " + feedUpdaters.get(feedName).url);
            }
    ...
    /**
     * Checks if any of the feeds should be updated base on the TTL and fetches. Returns true, if any feeds were
     * updated.
     */
